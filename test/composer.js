...
const { Composer } = Regraf

const baseMessage = { chat: { id: 1 }, from: { id: 42, username: 'regraf' } }
...
test.cb('Composer.acl should work with fn', (t) => {
  const bot = new Regraf()
  bot.use(Composer.acl((ctx) => ctx.from.username === 'regraf', () => t.end()))
  bot.handleUpdate({ message: { text: 'hello world', ...baseMessage } })
})
...
test.cb('should handle group command', (t) => {
  const bot = new Regraf(null, { username: 'bot' })
  bot.start(() => t.end())
...
test.cb('should handle command in group', (t) => {
  const bot = new Regraf('---', { username: 'bot' })
  bot.start(() => t.end())

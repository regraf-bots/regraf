...
test.cb('should provide shortcuts for `message` update', (t) => {
  ...
  bot.on('message', (ctx) => {
    ...
    t.true('getForumTopicIconStickers' in ctx)
    ...
    t.true('reopenGeneralForumTopic' in ctx)
    ...
    t.true('replaceStickerInSet' in ctx)
    t.true('setStickerEmojiList' in ctx)
    t.true('setStickerKeywords' in ctx)
    t.true('setStickerMaskPosition' in ctx)
    t.true('setStickerSetTitle' in ctx)
    t.true('setStickerSetThumbnail' in ctx)
    t.true('setCustomEmojiStickerSetThumbnail' in ctx)
    t.true('deleteStickerSet' in ctx)
...
test.cb('should provide subtype for `channel_post` update', (t) => {
  const bot = new Regraf('', { channelMode: true })
  bot.on('text', (ctx) => {
